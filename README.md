...
1. **[AD trust](docs/04-ad-trust.md)** — DNS and time, `ipa-adtrust-install`, `ipa trust-add`, then test Kerberos, `getent`, and group resolution.



### 3. Mapping

1. **[Analyze source LDIF](docs/08-analyze-source.md)** — inventory users/groups/sudo, catch duplicate POSIX ids, then **[map](docs/05-mapping.md)** (catalog, ID Overrides, HBAC, sudo).



### 4. Legacy enrollment

1. **[RHEL 6.10 client](docs/06-legacy-enrollment.md)** — enroll a legacy host and validate login as a **trusted AD user**.
...
| -------------------------------------------------- | -------------------- | ------------------------------------------------------- |
| *(none — Podman + ldapadd)*                        | 1 OpenLDAP           | Container, schemas, LDIF import                         |
...
**Copyleft** — GNU General Public License v3.0 or later ([GPL-3.0-or-later](https://spdx.org/licenses/GPL-3.0-or-later.html)).

You may run, study, share, and change this work. If you distribute it or a modified version, you must do so under the same GPL terms (source included). You may not relicense it as proprietary. Full text: [LICENSE](LICENSE).
